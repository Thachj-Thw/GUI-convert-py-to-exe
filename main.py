...
def click_convert():
    file_name.set(os.path.splitext(os.path.basename(path_input.get()))[0])
    command = f'python -m PyInstaller --clean --specpath ".\\spec" --distpath "{os.path.normcase(path_output.get())}" '
    if path_icon.get():
